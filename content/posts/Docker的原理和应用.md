...
{
  "registry-mirrors": [
    "https://t8svkgrf.mirror.aliyuncs.com"
  ],
  "debug": true,
  "experimental": false
}
...
- 调试、解决问题必备命令
>- 通常修改文件时会提示：read-only file system 或者 Permission denied挂载配置文件在运行docker容器时，可以将该配置文件复制一份到宿主机，然后run的时候，加载-v参数，将宿主机的配置文件挂载到docker中即可。
>- 以root权限进入docker容器
命令：
